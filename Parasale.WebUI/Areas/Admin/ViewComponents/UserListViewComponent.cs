...
{
    public class UserListForAdminViewComponent : ViewComponent
    {
        private UserManager<AppUser> _userManager;
        private IRepositoryWrapper _repository;

        public UserListForAdminViewComponent(UserManager<AppUser> userManager, IRepositoryWrapper repository)
        {
        ...
        private async Task<List<UserListViewModel>> GetItemsAsync(string userId)
        {
           ...
           // return await Task.FromResult(_userManager.GetUsersInRoleAsync(Role.User.ToString()).Result
            return _repository.UserRepository.GetUsersByAdmin(user.Id)
                .Where(p => p.IsManager == false && p.IsCompanyAdmin==false && (string.IsNullOrWhiteSpace(p.AssignedManager) || p.AssignedManager == userId))
