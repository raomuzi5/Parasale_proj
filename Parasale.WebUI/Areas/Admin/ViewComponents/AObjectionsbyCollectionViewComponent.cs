...
{
    public class AObjectionsbyCollectionViewComponent :ViewComponent
    {
        private IRepositoryWrapper _repository;
        private UserManager<AppUser> _userManager;

        public AObjectionsbyCollectionViewComponent(IRepositoryWrapper repository,  UserManager<AppUser> userManager)
        {
        ...
        private async Task<List<ObjectionViewModel>> GetItemsAsync(int id) {
            return await Task.FromResult(_repository.ObjectionRepository.getObjectionsbyCollection(id).Select(x => new ObjectionViewModel()
            {
                Id = x.Id,
