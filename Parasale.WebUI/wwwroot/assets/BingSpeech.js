...
// Based on Speech Services API documentation
...
(function (BingSpeech) {
    ...
    class Tools {
        /**
        * @param apiKey should be your Speech Services API key
        ...
        */
        ...
        constructor(apiKey) {
			//var apiKey="50a1989f87fe437b999051817e532e1a";
            this.authToken = "";
            this.canUseWebAudio = false;
            if (!apiKey) {
                throw "Please provide a valid Speech Services API key";
            }
            this.apiKey = apiKey;
            try {
            ...
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (this.authToken === "" || timeElapsed > 500) {
                    ...
                    try {
                        ...
                        console.error("Error issuing token. Did you provide a valid Speech Services API key?");
                    }
        ...
        */
        ...
        recognizeAudio(audio, locale) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                var textAnswer;
                var error = false;
                optionalHeaders = [{ name: "Content-type", value: 'audio/wav; codecs=audio/pcm; samplerate=16000' },
                    { name: "Authorization", value: "Bearer " + this._tools.authToken }];
                try {
                    var blob = new Blob([audio], { type: 'audio/wav' });
                    var requestParameters = "?language=" + locale;
                    response = yield this._tools.makeHttpRequest("POST", "https://westus.stt.speech.microsoft.com/speech/recognition/conversation/cognitiveservices/v1" + requestParameters, false, optionalHeaders, blob);
                    var parsedReponse = JSON.parse(response);
                    console.log(response);
                    if (parsedReponse.RecognitionStatus == "Success") {
                        textAnswer = parsedReponse.DisplayText;
                    }
                    else {
                        error = true;
                    ...
                    console.warn("Error while calling Speech Services Recognition API: " + ex);
                    let index = 0;
                    for (index = 0; index < this._actualRequests.length; index++) {
                        if (this._actualRequests[index].requestId === requestId) {
                            break;
